...
| **Owning Repo** | `exeris-docs` (the contract is organisation-wide, and so is `exeris-bot`; `exeris-inbox` is installed on the three data repositories only; `exeris-agent` on code repositories. The code lives in `.github`, which carries the stub) |
...
1. ~~**Two Apps**~~ **Three Apps** *(Amended 2026-09-19: `exeris-agent`, the hands — registered 2026-09-17, recorded here. See ## Amendments.)*, because a GitHub App's permissions are one set for every repository it is installed on — there is no per-repository narrowing, so "write to the inbox and nowhere else" cannot be one App with a write scope. **`exeris-bot`** is the *voice*: installed organisation-wide with `pull-requests: write` and `issues: write` and nothing else (`metadata: read` is implicit; it holds no `contents` scope at all), and it is the author of every automated review comment, label change and issue the organisation's workflows produce. **`exeris-inbox`** is the *pen*: installed only on `exeris-ai-execution` and its enterprise sibling, with `contents: write` and `pull-requests: write` there, and it commits rows and opens the inbox pull requests of §C.15. A third data repository joined the pen's reach on 2026-09-18: `exeris-ai-execution-streams`, private, holding what `execution.event_stream.ref` names — content, in ADR-086 §H.37's sense, which is why it is not a directory of the public inbox. `exeris-inbox`'s `contents: write` extends to it, so the capture producer, once it exists, can commit a stream and its index entry under the same identity that writes rows; `exeris-code-base.json`'s `repository_name.exclude` names it beside the two inbox repositories, and its own required check is the index-digest verification (`sha256` and `event_count` recomputed against the tree) — the streams analogue of §G.33's validator. **`exeris-agent`** is the *hands*: installed on code repositories and never on any of the three data repositories, with `contents: write` and `pull-requests: write` (`metadata: read` implicit), and it is the author of every commit and pull request an agent executes under the organisation's harness (RFC-2026-09-17) — a code-writing identity does not hold content, for the same reason the pen does not write code. A compromise of the first can talk; a compromise of the second can write to three data repositories; a compromise of the third can push a branch and open a pull request — it cannot merge over a code ruleset, cannot write a row, and cannot speak as the publisher. The three are one principle read three times — one App per write capability — and a fourth is a review-time question, not a default.
...
3. **Provenance survives a swap.** Every review comment carries a footer naming the runner — `provider`, model id, harness and version, the routine file and its SHA. The bot is the publisher; it is never described as the reviewer. ~~And ADR-085 §I.30's human accountability attaches to the pull request's author as it does today.~~ ADR-085 §I.30's human accountability attaches to the pull request's **owner** — its human author, or the `Owner:` a pull request authored by `exeris-agent` names (ADR-085 §I.30(b)). *(Amended 2026-09-19: author → owner, following the third App of §A.1. See ## Amendments.)*
...
8. **The publish step is the required check, and it is fail-closed.** It is green only when it parsed a schema-valid verdict with `decision ≠ BLOCKED` and no `checks_run[].result == not-run` for a gate the routine names as mandatory — or when the produce job was skipped by the deterministic path filter, which the log distinguishes from a crash. A verdict that is absent (runner crashed, timed out, cancelled by `concurrency`), invalid, `BLOCKED`, or resting on an unrun mandatory check is red. **A mandatory gate the verdict does not mention at all is `not-run`**: the schema requires `checks_run` to be non-empty and requires no particular entry in it, so silence about a gate and `result: not-run` are the same epistemic state and get the same answer — otherwise a review naming one gate it liked passes the check that was meant to notice. **And the deterministic skip is green only when the producing job SUCCEEDED and reported nothing to review.** A job that crashed before its own path filter ran emits no signal at all, and an absent signal read as "filtered out" turns every crash into a green skip whose log claims a filter it never reached. A label alone is never the gate: a rule that fails on `hard-block` present passes when the runner never ran. **Green is stated, never inferred.** What this clause lists is what is red today; the rule is what makes it a list rather than an enumeration somebody has to keep in step with reality. The check goes green only where something states it — a schema-valid verdict whose `decision` passes and none of whose findings carries `blocking: true`, or the deterministic skip whose producing job SUCCEEDED and reported nothing to review. Every other state is red, including a state this clause does not name: a skip kind nobody has seen yet, a decision a later schema adds, a marker recording that no review ran. A value that is merely not-red does not green a gate.
8a. **The produce job's two skip kinds.** It is skipped for a **draft** (skip kind `draft`, green — a draft cannot merge, so the state is stated) and for a pull request whose **author** login ends in `[bot]` (skip kind `bot-authored`, a standing verdict that is red when none exists — a run nobody human asked for is not a pass). *(Status 2026-09-17: this is the state `.github#62` landed. The target is a **sender** test, not an author one: an author is fixed for the pull request's life, a sender is the principal who acted, and only the second says whether a human asked for the review. That move waits on §C.14a's restore, per §A.5. See ## Amendments.)*
8b. **`[L2]` until the check exists — a `Bot`-authored pull request needs an independent human.** A pull request whose author is a `Bot` principal, and for which no approving review and no `l2-human-reviewed` record exists from a `User` principal, is red — whatever the verdict says. That `User` must not be the pull request's `Owner:` and must meet the `independent_reviewer` level `review-policy.json` declares, and must not have pushed to the branch where `exclude_pushers` is set (RFC-2026-09-17 §Recommendation). The rule lives here and not in a ruleset because it depends on the author, which a ruleset cannot see and a check can; it is what lets *required approvals* stay off in a solo organisation without a bypass actor. Four mutation cases: Bot author + no human event → red; Bot author + Bot approval → red; Bot author + User approval → green; User author + clean verdict → green. *(Amended 2026-09-19: see ## Amendments.)*
8c. **`[L2]` until the check exists — a review a `Bot` dismisses is still red.** A `review_dismissed` timeline event whose actor is a `Bot` principal is red for the pull request, with the dismissed review's author and state named. `pull-requests: write` permits an App to dismiss a human's *changes requested*, rulesets carry no *restrict who can dismiss* (that rule exists in classic branch protection only), and a block a human placed is not the execution identity's to lift. Four mutation cases: Bot dismisses *changes requested* → red; Bot dismisses *approved* → red (the approval no longer counts); User dismisses → not red on this rule; a push after approval → the approval no longer counts by the ruleset's own *most recent reviewable push* rule, and the check agrees. *(Amended 2026-09-19: see ## Amendments.)*
...
13. **One run record per L2 run, in the shape of `exeris-ai-execution/schemas/run-record.schema.json`**, assembled by the publish job from what CI already holds: the verdict; the check-run conclusions of the L1 gates as `oracle` state; `agent` from the runner's output (`model_id`, `model_snapshot`, `harness.client`, `harness.version`); `repository_state` from the checkout (`commit`, the bundle pin from `.agents/manifest.yaml`, `dirty: false`, `visibility` per §C.17); `accounting.mode` from the credential class; `execution` from the runner's execution log; `instrument` from the bot's own version and the fence in force. The bot adds nothing the schema does not name and interprets nothing — no aggregation, no comparison, no sentence about a model — per ADR-086 §A.2. The capture producer's first implementation is the by-hand deriver of ADR-086 Engineering Protocol 2, `tools/derive_ci_rows.py` in `exeris-ai-execution` — not a step in `docs-review.yml`. `docs-review.yml` gains the step only after the deriver has validated rows from the rescued streams, because a derivation added to a live workflow before it has been run over known input is a second untested producer beside an empty inbox. When it lands in the workflow, the stream commits to the streams repository (§A.1) by `exeris-inbox` **directly on its main branch**, and the row's `execution.event_stream.ref` names that commit: a stream is content, not a row, and a pull request there would leave the ref dangling until merged, which §C.15's own rule — the inbox holds rows, quarantine is a producer-side state — does not admit for a reference the row depends on. *(Amended 2026-09-19: see ## Amendments.)*
14. **Producer-side derivations are this ADR's, not the schema's** (ADR-086 §C.12–14 and §F.30–31 name the fields; this names how a CI producer fills them):
    - `agent.system_prompt_sha256` — SHA-256 over, in this order and each terminated by a newline: the prompt text the workflow passed to the runner; the routine file at the checked-out `.github` SHA; `AGENTS.md` **as the runner read it**. The hash records what happened, not what should have; which `AGENTS.md` the runner reads is §C.14a's rule. The harness's own prompt changes are covered by `harness.version`. A runner that does not expose enough to compute this yields **no row**, not a row with a convenient hash. A backfilled row (ADR-086 Engineering Protocol 2) has no prompt text a runner exposed; there the hash is **reconstructed** from what GitHub still holds — the `docs-review.yml` template at the SHA `referenced_workflows` names, the caller's `guardrails.yml` at head, the L1 job conclusions, the routine file, and `AGENTS.md` at head — rendered through the same substitution the live path uses. That is honest only as a stated derivation, never as a capture: the row's `instrument.fence` names it a reconstruction, and the fence stands until the first live run that exports these same components lets the two be checked component-by-component; a disagreement retires the fence by a dated entry and the rows already written stay marked, never silently re-derived. A component the reconstruction cannot recover exactly still yields **no row**. *(Amended 2026-09-19: see ## Amendments.)*
    ...
    - `execution.event_stream` — the runner's execution log, uploaded as a *separate* workflow artefact (it contains content), referenced by artefact URL, SHA-256 and event count. Never copied into the row, never committed to an inbox. The artefact sits on a public repository for `retention-days` before anything commits it to the streams repository of §A.1, and for that window it is a workflow artefact like any other: anonymous download is refused, any signed-in account's is not. Two remedies exist — delete the artefact once committed, or write to the streams repository directly from the produce job and upload none — and neither is decided; the record states the exposure here rather than in a README. *(Amended 2026-09-19: see ## Amendments.)*
...
14a. **The instruction files the runner reads come from the base branch, not the pull request's head**, ~~whenever the pull request's author is not a member of the organisation~~ for every pull request, whoever its author *(amended 2026-09-26)*. The produce job restores `AGENTS.md` at any depth, `.agents/**` and every provider adapter (`.claude/**`, `.codex/**`, `.cursor/**`, `.gemini/**`, and the vendors' single-file instructions) from the base branch (~~from the merge base~~) before the runner starts, keeps what the pull request wrote beside the checkout for review and never executes it, and names those paths to the reviewer; a pull request from outside that touches those paths is reviewed by a human before any runner reads it. ~~Today every pull request is the maintainer's and this costs nothing; the rule is written now because it matters exactly when the §E.26 trigger fires and there is no time left to write it.~~ The organisation's execution identity, `exeris-agent[bot]`, is the first author the restore admits to review; every other bot author is still not reviewed. `agents-md-schema.md` rule 8 is the reasoning: instructions an agent follows are pinned and reviewed, and a head-branch `AGENTS.md` from a stranger is neither — nor is one from the agent whose pull request is under review. An App principal is not a member, so this clause binds every pull request `exeris-agent` opens. It is not implemented (`docs-review.yml`'s produce job: the `[bot]` author test of §B.8a is what keeps its trigger closed). Until the restore exists, the produce job does not run on an App-authored pull request and the human door is the only review; the change that implements the restore is the change that removes the author test — the two land together or not at all, because an agent reviewed against rules it rewrote in its own pull request is worse than an agent not reviewed. *(Amended 2026-09-19: see ## Amendments.)*
...
20. **Every issue names its human.** The body opens with the merged pull request, its author and the person who merged it; the bot is the pen. ADR-085 §I.30 speaks of agents, and this workflow is deterministic — the rule's spirit is honoured anyway, because the first automated issue without a human name is how the rule erodes. A pull request the hands authored names its owner the same way, and `pr_body_check.py` gates it. *(Amended 2026-09-19: see ## Amendments.)*
...
- **Replacing review.** The bot is transport from a verdict to its consequences. A human still reads the review, and ~~ADR-085 §I.30's accountability is unchanged~~ ADR-085 §I.30's accountability is unchanged in force; its field moved from author to owner by ADR-085's amendment of 2026-09-19. *(See ## Amendments.)*
...
- **Risk:** the bot becomes the third home for rules — the pull will exist on every review that finds something CI could have caught. §E.27 is the answer every time, and `.github`'s own `guardrails.yml` checks the two files it may have. The maintainer notices first, which is the problem ADR-086 names too.

## Amendments

- **2026-09-26 — §C.14a is implemented, for every author, and the execution identity's pull
  requests are reviewed.** The produce job in `exeris-systems/.github` skipped every pull request a
  bot opened, because §C.14a was not implemented and a runner reading an agent's own `AGENTS.md`
  would review the change under instructions the change wrote. The organisation's execution
  identity now opens pull requests that are the organisation's own work, and none of them was read
  by the review. The restore now runs before the runner starts on every pull request: the runner
  puts its own configuration back itself, and a step of the organisation's puts back `AGENTS.md`
  at any depth, `.agents/**` and the other vendors' instruction files, keeps what the branch wrote
  under `.exeris-pr/`, and names those paths to the reviewer. It runs for every author rather than
  only for non-members, for the reason the repository extension is read at the base for everyone:
  a pull request does not write the rules its own review follows, and a member's pull request is
  no exception. The content restored is the base branch's, which is what the runner restores its
  own paths to, rather than the merge base's. The produce job admits `exeris-agent[bot]` as an
  author and no other bot, and `principal_check.py` refuses that admission without the restore
  step, so the two cannot come apart. Alternative considered and rejected: restoring only for bot
  authors — the rule would then depend on a classification of the author, where it holds
  whatever the author is. Not decided here: whether a pull request the identity authored also
  needs a person's approval before the required check is green (RFC-2026-09-17, §B.8's
  author-conditional red); the merge remains a person's act.

- **2026-09-19 — §B.8 gains two RFC-dependent reds: an author-conditional one, and a dismissal one.**
  Both assume `exeris-agent` (§A.1) authoring pull requests a human did not necessarily review line by
  line, which the door and the readiness check (2026-09-17, below) do not by themselves prevent — a
  `User` sender can start a review of a `Bot`-authored change and never read it. §B.8 gains a rule that
  reads the pull request's *author*, not its sender (§B.8b): a `Bot`-authored pull request is red
  unless an independent `User` approved it or applied `l2-human-reviewed`; and a `Bot` that dismisses a
  human's review is red regardless of what it dismissed (§B.8c). Neither exists in `publish_verdict.py`
  today — `review-policy.json` is RFC-2026-09-17's, not built — so both are marked `[L2]` and wait on
  the policy file the RFC defines. Alternative considered and rejected for the first: a
  required-approvals ruleset — GitHub's own semantic has no *author is a Bot* condition, so it would
  apply to every pull request or none.

- **2026-09-19 — a third App, and *author* becomes *owner* in three sentences.** §A.1 split two Apps
  by what they may write and left code unwritable by any of them, which was right for a publisher and
  a row writer and left every interactive agent commit under the maintainer's login. RFC-2026-09-17
  adds the hands on §A.1's own principle — registered 2026-09-17, ahead of this text saying so. Three
  sentences in this record said the human *author* is accountable; they now say the *owner*, because
  the App is the author of what it executes and the human is accountable for it either way — the
  substance of ADR-085 §I.30 kept, the field it lives in changed and gated. §C.14a gains one sentence
  stating a property it already had. Alternatives considered and rejected: widening `exeris-bot` with
  `contents: write` (the "single App carrying `contents: write` everywhere" §A.2 admits only if this
  record stops calling its scope minimal — it does not); a machine user (Alternatives Rejected,
  unchanged).

- **2026-09-19 — the pen gains a repository, the hands lose one, the capture producer gets its first
  form, and two open questions are stated rather than left to be discovered.** §A.1 named two data
  repositories because two existed. The rescue of the first 76 streams on 2026-09-18 created a third,
  private by construction: `exeris-ai-execution-streams`. Every rule that mentions "the two inbox
  repositories" — the pen's scope, the hands' installation list, `exeris-code-base.json`'s exclusion,
  the base ruleset's "no rules" finding on 2026-09-18 — now reads three; the hands are installed on
  none of them, for the reason a code-writing identity does not hold content. §C.13 gains the fact that
  the capture producer's first implementation is a by-hand deriver, `tools/derive_ci_rows.py` in
  `exeris-ai-execution`, run once over the rescued streams — not a step in `docs-review.yml`, because a
  derivation added to a live workflow before it has been run over known input is a second untested
  producer beside an empty inbox; the workflow step follows only once the deriver has validated rows,
  and from then on the stream commits to the streams repository directly on its main branch, because a
  stream is content and a pull request would leave the row's own reference dangling until merged. That
  deriver reconstructs `agent.system_prompt_sha256` from what GitHub still holds rather than from a
  prompt no stream carries; §C.14's bullet now says so and says it is honest only as a stated
  derivation, fenced, and checked component-by-component against the first live run that exports the
  same components — a disagreement retires the fence, not the rows. §C.14's `execution.event_stream`
  bullet gains the fact that the seven-day artefact window is exposed to any signed-in account and that
  neither remedy for it is decided. Alternative considered and rejected for the repository: a
  `streams/` directory inside `exeris-ai-execution` — the public inbox is closed to content "in any
  form" and the split is the reason the repository exists.
...
- **2026-09-17 — §B.8's fail-closed check has one door, and this record did not describe it.**
  ...
  **What it is.** A person reviews the change themselves and applies the label. The publication
  ...
  commit it names, so the next push leaves it behind and the check is red again. The record greens the
  check only when the label's sender is a `User` principal. A label applied by a `Bot` principal — any
  App the organisation installs, `exeris-bot` included, and any App a pull request's own workflow could
  act as — is taken off and recorded as refused, with the sender named. "Anyone can re-apply the label"
  was written about people; the check that reads the record now says so.
...
- **2026-09-17 — the door checks who knocked, and the record catches up with code already shipped.**
  The `l2-human-reviewed` record named the applier and the commit and did not check the applier's
  type, so any App with `pull-requests: write` — the voice has it — could green the required check by
  applying a label. No App did; the clause is closed because the next App the organisation adds
  (`exeris-agent`, the hands, §A.1) holds that scope on every code repository and its entire reason to
  exist is that it *cannot* establish "a human verified this". `publish_verdict.py`'s
  `human_principal()` refuses a `Bot` sender and `principal_check.py` records the refusal; both
  existed in `exeris-systems/.github` before this record said so. Alternative considered and rejected:
  a per-App allow-list — a list is a thing somebody adds an App to; a principal type is not.

- **2026-09-17 — the bot test moves from the author to the sender, and one green skip becomes two —
  landed as `.github#62`.** Measured against the checkout: `docs-review.yml` skipped produce when the
  pull request's author login ended in `[bot]` and reported `draft-or-bot`, which `publish_verdict.py`
  listed beside `fork` as a skip that is green on its own. That was written for pull requests no human
  would ever ask a review of. It is wrong the moment an execution identity authors pull requests a
  human *does* ask a review of: the author stays `exeris-agent[bot]` after the human marks the draft
  ready, so the produce job is skipped, the skip is green, and the required check passes on a pull
  request nothing read. `.github#62` splits the one skip into two — `draft` (green, a draft cannot
  merge) and `bot-authored` (a standing verdict, red when none exists) — which closes the immediate
  hole without yet reading the sender: the test still asks who the author is, not who acted. The move
  from author to sender waits on §C.14a's restore (§B.8a, §A.5): an agent-authored pull request needs
  the base branch's instructions before the produce job may run on it at all, and that restore is not
  yet built. Verified by `publish_verdict_suite.py` (131 cases) and `principal_check.py`, which reads
  the shipped expressions. Consequence for Dependabot-class pull requests: they lose the automatic
  green and wait for a human readiness click. A named green-skip list was considered and rejected for
  the reason the door's allow-list was — a list is a thing somebody adds an App to.

- **2026-09-17 — readiness starts from a `User`, and the record catches up with code already
  shipped.** §B.8's 2026-09-16 amendment named which events start a review — `opened`, `reopened`,
  `ready_for_review`, a label — and said nothing about who may cause them. "The bot never applies the
  request label" was a policy statement about the one App that existed; with `exeris-agent` able to
  open pull requests (§A.1) it is a check, and the same check as the door's (above): the readiness
  label greens nothing unless its sender is a `User`, and `opened`/`reopened`/`ready_for_review` start
  a review only when the event's actor is a `User`. `principal_check.py` reads this from the same
  expressions the door uses, and it was in force before this record named it.
...
- **2026-09-16 — §B.8 states the rule the four fail-opens were instances of: green is stated, never
  inferred.** The clause enumerated what is red, and in one day four values that were neither a
  verdict nor a red were read as a pass. An empty `produce-relevant` read as "the filter ran and
  found nothing". A run whose actor was the publication's own bot read as "this event says nothing
  about the pull request, therefore fine". A `decision=NONE` marker — the one the publication writes
  precisely when NO review ran — read back as a standing verdict that passes. And a finding marked
  `blocking: true` beside a `CONDITIONAL` decision, where the gate read the decision and never the
  finding. The first three took the absence of information for the absence of a problem; the fourth
  took an explicit stop for consent. Each was patched after it bit, on a live pull request, and
  three of the four surfaced because somebody looked at a green check that felt wrong — not because
  anything reported them. Enumerating the red states is what let each one arrive as a new case, so
  the clause now carries the rule as well as the list.
...
- **2026-09-16 — when the review runs becomes readiness, and §B.8 gains staleness.**
  ...
  **A verdict covers the commit it reviewed, and nothing noticed when the head moved.** A pull
  ...
  `synchronize` or `edited`. The readiness label starts a review only when its sender is a `User`;
  `opened`, `reopened` and `ready_for_review` start one only when the event's actor is a `User`.
  "The bot never applies the request label" was a policy statement about one App; with a second App
  able to open pull requests (§A.1) it becomes a check, and it is the same check as the door's
  (above).
...
1. **Register the three Apps** of §A.1 — `exeris-bot` organisation-wide, `exeris-inbox` on the three data repositories only, `exeris-agent` on code repositories only; store the private keys as organisation secrets; add `EXERIS_FINGERPRINT_KEY`. Record the three dates under `## Amendments` as the rotation baseline. *(Amended 2026-09-17: no `EXERIS_FINGERPRINT_KEY` — the `ci:` derivation carries no key, so there are two dates, not three. See ## Amendments.)* *(Amended 2026-09-19: the third App and the third data repository — see ## Amendments.)* Registration needs organisation-admin rights and a browser; nothing before it needs a token — the composed schema, the label mapping, the routine amendment and the produce→publish split (items 2–3) land first, by pull request, and run under the provider's identity until the Apps exist.
