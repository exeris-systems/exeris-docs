...
23. ~~**The human arm runs first.**~~ **A group is read against its own arms; a human arm is declared only where the question is about the human.** ~~`human_baseline` is a field on each model row, not a row of its own, so it is known when each model row is written and no row is rewritten later; and the human has not seen a model's output. A group declares `pairing.baseline: human | none` when it is planned; `human` requires `human_baseline` on every row, `none` is admitted and marks a group that can never carry an economic claim — a cost figure with no human reference point is not interpretable, only quotable.~~ A group declares `pairing.baseline: human | none` when it is planned, and `none` is the default. Within a group every arm ran the same task from the same commit under the same oracle, so each arm is the reference point of every other: an arm's cost-to-true-done read against another arm's, in the units §E.21 admits across models, is interpretable, and a group of model arms carries that comparative claim without a human in it. What a group with no human arm cannot carry is the one claim that is about the human — that delegating the task costs less than doing it. A group that wants that claim declares `human`, and then the rules that made the field sound still hold: `human_baseline` is a field on each model row, not a row of its own, so it is known when each model row is written and no row is rewritten later; the human arm runs before any model arm, so the human has not seen a model's output; and every row of the group carries it. The moment `human_baseline` needs a second field about the *human* rather than about the *work*, it stops being a field and becomes a record; that is a MAJOR change and is anticipated here so that it is recognised rather than discovered. *(Amended 2026-09-23: see ## Amendments.)*
...
- **2026-09-23 — a group is read against its own arms, and the human arm becomes the exception.**
  §E.23 made the human the reference point of every group and admitted `none` only as a group that
  "can never carry an economic claim". That premise does not hold for the question this layer is
  asked most: which of several models reaches `TRUE_DONE` on a task, and at what cost. The arms of a
  declared group share the task, the starting commit and the oracle, so each is a reference point
  for the others, and a comparison between them in USD and wall time (§E.21) is as interpretable as
  one against a person — the V4 criterion of §B.6 already reads a router against a frontier arm,
  not against a human. What only a human arm answers is whether delegating the task pays at all,
  which is a question about the person's time rather than about the models. The clause now makes
  `none` the default, admits the comparative claim across arms, and keeps `human`, with its
  ordering and byte-for-byte rules unchanged, for a group that asks the delegation question.
  Measured on the first planned group: a four-file frontmatter backfill with one link stub, whose
  human arm would have timed a task nobody doubted a person could do, and would have blocked every
  model arm until it had. Alternative considered and rejected: a named reference arm per group —
  a field that picks one arm as the standard reads as a judgement about that model, which §B.5
  forbids; every arm being the reference of every other needs no field at all. Nothing in
  the schema changes: `pairing.baseline` keeps both values and only its default moves, so no fence
  is written.
