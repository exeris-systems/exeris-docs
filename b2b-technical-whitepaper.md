...
<!-- VERIFY(sweep-2026-09): §3.2 says "SKUs compose 10–15 capabilities" while §3.3's representative manifests list 7 to 16 named capabilities each (Bot Blocker 7, IDP/PIM/OMS 16). The manifests are explicitly excerpts, so they do not settle the range, and no exeris-sku-* repository exists to count a real manifest against. The identical sentence is at high-level-architecture.md:190 and in both auto-fetched portfolio copies, so any change is a four-file edit. A human must decide the range and move all four together. -->
...
<!-- VERIFY(sweep-2026-09): the "Commercial positioning" column below asserts comparative claims no campaign supports — "Envoy/Kong density and policy parity at sub-Envoy memory and CPU footprint" (API Gateway), "Sub-millisecond P99 failover at the edge" (Edge Proxy), "Page-throughput parity with hyperscaler IDP at fraction of per-page cost" (IDP). No exeris-sku-* repository exists in the checkout, and exeris-benchmarks carries no edge, gateway or failover scenario, so none of the three is measured. These are marketing-positioning figures CLAIMS.md does not gate; a human must decide whether the column states targets, drops the comparators, or waits for §4.3's campaigns. Changing one cell and not its siblings would read as if the others had been checked. -->
...
<!-- VERIFY(sweep-2026-09): §4.4 describes perf-box-amd64 as "Hetzner AX-class — AMD x86-64, 16 hardware threads, 64 GB RAM; Falkenstein DE / Helsinki FI". exeris-benchmarks/docs/hardware-profiles.md's perf-box-amd64 section does NOT pin those values — it records CPU model, cores, threads and RAM as per-run fields with unrelated examples (AMD EPYC 7763, 128 GB). The concrete description traces only to exeris-kernel/docs/whitepaper.md §5.2; the one published campaign on the profile records "AMD Ryzen 7 7700 (8C/16T), 62 GB RAM … Hetzner" and no datacentre. A human must decide whether the profile document should pin the hardware or the whitepaper should stop describing it as fixed. -->
