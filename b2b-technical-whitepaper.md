---
title: "Exeris: B2B Technical Whitepaper"
type: explanation
visibility: public
...
| **Enterprise Kernel** | `exeris-kernel-enterprise` | Alternative substrate driver (`exeris-kernel`-SPI implementation) shipping `io_uring` (Linux) / IOCP (Windows) transport, `EnterpriseQuicTlsEngine` (QUIC TLS via OpenSSL `BIO_DGRAM` pair), HTTP/3 codec, slab-pool memory; NUMA-aware allocation via `libnuma mbind()`, specified in the Enterprise memory subsystem document (enterprise-private per ADR-020). Activated by Maven-coordinate swap with the open-core `exeris-kernel` Community driver (custom NIO H1/H2 + portable Off-Heap TLS over TCP). |
| **Exeris SDK** | `exeris-sdk` | `@ExerisDomain`, `@Action`, `@Field`, `@Relationship` annotations; source-model AST; Tailwind UI kit. Build-time only — no runtime coupling (ADR-003). Method-level RBAC is declared with the kernel-SPI `@RequiresRole` (ADR-014). |
...
| **Telemetry & Observability** | `exeris-telemetry-spec`, `exeris-enterprise-observability` | JFR-first wire format plus decoder/CLI/forensics per ADR-018 — both repositories are enterprise-private today; ADR-018 records the wire format as publishable as an open spec, and that has not happened yet. Replaces sidecar-based APM. |
...
|  | `exeris-caps-outbound-credentials` | commercial | Cross-cutting — not confined to one SKU family (HLA §3.2 layer 7) |
|  | `exeris-caps-service-identity` | commercial | Cross-cutting — not confined to one SKU family (HLA §3.2 layer 7) |
|  | `exeris-caps-idempotency` | commercial | Cross-cutting — not confined to one SKU family (HLA §3.2 layer 7) |
|  | `exeris-caps-usage-metering` | commercial | Cross-cutting — not confined to one SKU family (HLA §3.2 layer 7) |

One capability repository exists today — `exeris-caps-cors-policy` (`community`, Apache 2.0), at **scaffolded** status per [`cap-license-registry.md`](cap-license-registry.md). Every other capability repository is in **specified** status. Implementation cadence is driven by the SKU roadmap in §7. The Capability Composition Model is governed by **ADR-024** (accepted 2026-05-13, amended through 2026-07-21), the licensing taxonomy by **ADR-023** (accepted 2026-05-13), and the SKU composition manifest format by **ADR-053** (JSON, 2026-07-21); the codegen pipeline (ADR-015) and the kernel bootstrap lifecycle contract are the implementation substrate those ADRs bind.
...
|---|---|---|---|---|
| **API Gateway** | Gateway | Source-available (Exeris Commercial License) | API-first B2B platforms running gateway infrastructure at production scale | Policy chains and routing run in the application process, with no sidecar and no separate proxy tier to operate |
| **Edge Proxy** | Gateway | Source-available (Exeris Commercial License) | Multi-region SaaS, CDN-adjacent deployments | Failover decided in-process at the edge, and telemetry emitted without sidecars |
...
| **IDP** | Service Boundary | Source-available (Exeris Commercial License) | Document-heavy B2B operations (insurance, logistics, legal) | Document processing that deploys on-premises, inside the customer's own boundary |
...
| A server-backed deployment carries its coordinator process whether or not a saga is running; an in-process engine has no at-rest cost to measure | **not yet measured** — the whole-deployment Σ RSS footprint rollup that would measure it is recorded `deferred` in `scenarios/e2e-shop-order-saga/CONTRACT-v2-IMPLEMENTATION.md` §8, and that ledger's own rule is that no claim may depend on a `deferred` row | — |
| Comparative saga throughput, latency or compensation correctness under contract v2 | **not yet measured** | — |

The coordinator-at-rest question is the one that carries the architecture, and it is a property of the *label* rather than of a product: the point is not that one vendor's server is heavy, it is that a coordinator process is a resident cost the application pays whether or not a saga is running. An in-process engine has no at-rest cost to measure, because there is nothing running to measure. **No campaign has measured that cost under contract v2, and the whole-deployment footprint rollup that would measure it is recorded `deferred` in `CONTRACT-v2-IMPLEMENTATION.md` §8 — so this section states the structure and not a multiplier.**
...
Engine-level comparator on the **B5 Memory-BIO harness** (comparator labels and wiring caveats defined in `exeris-benchmarks/docs/tls-zero-copy-benchmark-matrix.md`). The Exeris row exercises `OffHeapTlsEngine` from `eu.exeris.kernel.core.crypto.*` — a Core-shared engine; the published report (`results/reports/20260501-123118-all/aggregate-20260501-123118-all.md`) classifies the B5 row as **Enterprise tier** and stamps its baseline rows `comparison_eligible`. B5 is an in-process Memory-BIO engine-level lens (no socket, no syscall) and is deliberately not equivalent to the FD-owner socket integration path (B6) — the two are reported as separate rows, never collapsed into one equivalence claim.
...
At this layer the **Code Detachment Fee** is the one-time license that transfers SKU ownership permanently to the customer's repository fork (ADR-023 §"SKU Repository Source-Visibility Policy", obligation 9). The fee scales with deployment size and SLA tier and is set under Exeris's internal commercial policy; ADR-023 is deliberately silent on pricing and this document follows it. This is the structural realization of the IP sovereignty promise from §6 — paying the fee unlocks not a runtime privilege but a property right.
...
**Sovereignty extends to the SKU layer.** When a customer detaches a vertical SKU under the §5.4 Code Detachment model, they receive (a) the substrate code their tier already entitles them to, (b) the SKU's cap composition manifest, (c) the SKU-specific policy capabilities they are licensed for, and (d) the build-time codegen artifacts (`exeris-tooling` outputs) needed to operate the SKU independently of further Exeris updates. The fee is positioned as the cost of a transferable property right, not a barrier to exit; its level is set under Exeris's internal commercial policy, which this document does not restate. Customers who pay it become structurally independent of Exeris Systems for the SKU they detached.
...
| Horizon | Milestone |
|---|---|
| Q3 2026 | Kernel v0.8 → **v0.11 shipped** (v0.11.0, 2026-08-11) — quality gates closed; **TRL-4 integration-tested** for the Crypto subsystem, the only one at that level today (`exeris-kernel/docs/subsystems/crypto.md`); the rest of the kernel subsystem set is TRL-3; first internal Edge/IoT pilot cohorts begin against `exeris-kernel-community` |
...
**All future Family products will be pure-Exeris**, built on `exeris-sdk` (`@ExerisDomain`, `@Action`, `@Field`, `@Relationship`) + `exeris-tooling` codegen + the Tier 2 capability ecosystem, with method-level RBAC declared through the kernel-SPI `@RequiresRole` (ADR-014). BudgetHQ remains the singular Spring-on-Exeris product because its dogfooding role is to validate `exeris-spring-runtime` itself; that role is filled once and does not need to be repeated.
...
Each of these capabilities lands in the platform's capability ecosystem **after** BudgetHQ has stabilized them in production — never before. BudgetHQ is not built to prove the platform works; the platform is structurally sound enough that BudgetHQ can run on it from day one, and that is the proof. The capability development pipeline — prototype under real production load, promote once stable — is what the Family-product relationship is for.
