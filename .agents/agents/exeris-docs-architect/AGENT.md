...
model: strong
skills: [exeris-docs-three-tier-narrative-review, exeris-docs-drift-pattern-sweep-review]
policies: [drift-patterns, editing-large-documents, adr-registry]
references: [three-tier-architecture, capability-layer, graph-subsystem, bootstrap-dag]
handoffs:
  - {agent: exeris-docs-adr-registry-keeper, when: "an ADR's number, location or taxonomy is touched", blocking: true}
  - {agent: exeris-docs-implementer, when: "a finding is confirmed and needs applying", blocking: false}
