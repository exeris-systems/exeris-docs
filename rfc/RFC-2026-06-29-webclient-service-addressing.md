...
| **Owning Repo**   | `exeris-kernel` (the addressing seam lives on `KernelWebClient` / the `HttpClientEngine` SPI); this RFC is hosted in `exeris-docs` because the decision binds `exeris-tooling` too |
| **Target ADR(s)** | TBD — a kernel-scope "WebClient service addressing" ADR once accepted; number reserved in [`adr-index.md`](../adr-index.md) **only** when the implementation build gate opens (not at RFC time — RFCs carry no registry number) |
| **Affected Repos**| `exeris-kernel` (the resolution seam + any Community driver + the `KernelWebClient` consult point), `exeris-tooling` (T12 — generated clients emit a logical peer name instead of a hard-coded host), `exeris-docs` (this RFC + the eventual ADR). Enterprise registry/mesh drivers are out-of-repo and are referenced descriptively only. |
| **Reviewers**     | —                                                                    |
...
Several cross-cutting constraints bound any answer and must not be lost in the strategy debate: resolution must compose with the request enricher (ADR-032) and the v0.9 IDP outbound-credential direction so that **identity survives re-addressing**; the failure modes (name unresolved / no healthy endpoint / resolution timeout) must be classifiable; whether resolution is per-call or cached-with-TTL must be decided; and **The Wall** must hold — no framework DI, no `ThreadLocal`, and resolution must not couple the client to a concrete registry/mesh type.
...
- **Within the ecosystem (decided):** ADR-034 (`KernelWebClient` tier-neutral facade over `HttpClientEngine`, superseding ADR-026's `CommunityWebClient`); ADR-032 (`HttpClientRequestEnricher` — implicit tenant/principal/trace propagation on outbound requests); the v0.9 "Security: `IdentityProvider` SPI Direction — RFC Track" (outbound-credential touchpoint). The kernel already resolves *every* driver via `ServiceLoader` + the Open-Core SPI/Core/Driver split — a pluggable resolver would be idiomatic, not novel.
...
**Pros:** Zero new runtime dependency; no liveness machinery; trivially Wall-clean; works in every environment including local dev; matches 12-factor config.
**Cons:** No dynamic membership or health — topology changes need a reconfig/redeploy; no load-balancing across replicas beyond what a single endpoint encodes; does not scale to a dynamic mesh.
...
**Pros:** Standard and env-provided; no kernel registry; SRV carries port + weight + priority for client-side balancing; the topology lives in the platform's existing DNS.
**Cons:** DNS/JVM caching and TTL semantics are subtle (stale endpoints, negative caching); no application-level health beyond what DNS removal provides; SRV is not universally provisioned; ties the kernel to DNS as the resolution substrate.
...
**Pros:** The only option under which the *same* generated client runs unchanged across dev (static), k8s (DNS-SRV), enterprise (registry), and mesh (a pass-through driver) — the idiomatic kernel `ServiceLoader`/Open-Core model; subsumes A and B as drivers rather than competing with them; Wall-respecting (a resolution interface, not a concrete registry type); the natural seam for caching/TTL, health, and failure-mode classification to live behind one contract; future registry/mesh-aware drivers slot in without touching `KernelWebClient`.
**Cons:** A new SPI surface to design, TCK, and maintain — the heaviest upfront cost; needs an explicit failure-mode taxonomy, a caching/ownership contract, and a defined ordering against ADR-032 enrichment + IDP credentials; risks over-engineering if T12 only ever needs a static map.
...
**Pros:** Zero kernel work; leverages a mature data plane (mTLS, retry, telemetry "for free"); no new SPI surface.
**Cons:** Hard dependency on a mesh being deployed — abandons every non-mesh deployment (local dev, simple k8s, edge, single-box); the logical-name → local-endpoint mapping still has to come from *somewhere* (env/sidecar config), so the problem is displaced, not solved; the platform loses the ability to let a generated app **self-describe its peers**, which is core to the composable-unit / marketplace direction.
...
**Cons:** T12 stays blocked at the kernel seam; the multi-application mesh direction the ecosystem is committing to cannot be demonstrated end-to-end. Not acceptable as a standing answer — which is why this RFC exists rather than being withdrawn.
...
- **`HttpClientEngine` binding model (implementation crux).** `KernelWebClient` holds one engine bound to one host. Does resolution (a) hand the engine a resolved endpoint per `send`, (b) maintain a per-host engine/connection-pool behind the resolver, or (c) make the client hold a resolver + an engine factory? This is the real design question for the resulting ADR and the subject of the pre-ADR spike. — owner: `exeris-kernel` HTTP subsystem.
...
- **Identity / IDP audience binding across re-addressing.** Lock the resolve → enrich → send ordering and the audience-binding rule against the v0.9 IDP outbound-credential RFC. — owner: security + HTTP, resulting ADR.
- **Failure-mode taxonomy → error codes.** Define distinct codes for unresolved-name / no-healthy-endpoint / resolution-timeout. — owner: resulting ADR.
