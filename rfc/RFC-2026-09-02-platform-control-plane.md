...
1. **CMS authoring surfaces.** Content editing holds no customer-infrastructure credential, so the C test puts it public while Studio stays private. That is not a contradiction, and the separation has a sharper test than the credential one.
   ...
   **CMS does not need the composition graph; Studio is the surface that reasons about it.** Studio's subject is `@Provides` / `@Requires`, the cap DAG and the Wall — ADR-024's graph. A content author never touches it. Both still ride the same metadata artifact, so this separates the *surfaces* without forking the *model*.
   ...
   The integration this raises — author a static part, model with caps/SKUs, have Studio compose backend and frontend — is already designed and does not need re-deciding here. [The presentation/front RFC](../../exeris-sdk/docs/rfc/RFC-2026-06-25-presentation-front-model.md) fixes it: a content type **is** an `@ExerisDomain` (content modelling is Entity-First), what a CMS additionally needs is *composition* — pages, sections, slots, authored regions — and that is the presentation IR, riding **one** `exeris-metadata/*.json` artifact with **two authoring front-doors**: the Java annotation lane for backend-anchored views, and a non-Java lane (Studio, or hand-authored IR) producing the same `ViewMetadata`. A backend-less authored page is first-class there (`source = STATIC` / `NONE`), not a degenerate case. `exeris-codegen-ts` already recognises the `view` family alongside `domain` and `enum`.
