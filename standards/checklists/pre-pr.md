...
2. **The Wall.** Does any import cross an edge that `ecosystem-map.md` forbids? Did I say `none` because I checked, or because it is the default?
3. **Category B.** Did I touch a `@Generated` file by hand? (Then stop.)
4. **TCK.** If observable SPI behaviour changed, which `Abstract*Tck` covers it, and which Community binding test runs it?
5. **Compatibility.** Did japicmp run, and does the PR's *Compatibility impact* line match its report?
