...
8. **Template sections** (added to `templates/ADR-TEMPLATE.md` and `RFC-TEMPLATE.md` on 2026-09-04): ADR carries `### 🚫 Non-Goals` and `### ⚠️ Risks and Assumptions` under *Consequences*, matching the decorated form the corpus already uses for `### 🏁 The Decision`, `### ✅ Positive Outcomes`, `### ⚠️ Trade-offs` and `### 📋 What is NOT in scope`; an RFC that proposes an SPI surface carries `## Testing`. Absent sections fail review. `[L2]`
