...
8. **Links:** intra-repo links are relative, to the `.md` file (the site rewrites them). A cross-repo link is an absolute `https://github.com/exeris-systems/<repo>/blob/<branch>/<path>` URL that names the branch carrying the file. A sibling-layout path (`../../exeris-kernel/docs/adr/ADR-065-….md`) resolves only where every sibling repo is cloned beside this one, so it is dead on github.com and in a single-repo CI checkout; the site restores in-site navigation by rewriting the absolute form to an internal route at build time (ADR-085 §A.2, as amended 2026-09-05). A private repo is never linked (rule 6). Links to a repo-root file (`CONTRIBUTING.md`, `MIGRATION.md`) are allowed only for files on that repo's whitelist in `exeris-docs/site/sources.yml`. `[L1: lychee]`
