...
- Is this page a record — an ADR or a `standards/` page? Then no figure belongs on it at all, however well sourced: cite the report instead. Rule 6 overrides rule 1's citation allowance here, and asking rule 1's question first is what let sourced figures accumulate in one ADR.
