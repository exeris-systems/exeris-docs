...
4. **The registered drift patterns are lint.** The figures and phrasings withdrawn in `exeris-docs/CLAUDE.md` *Common drift patterns* (items 10–13) are Vale `existence` rules at **error** level — the one place Vale is allowed to fail a build. `[L1: vale Exeris.RetractedFigures]`
