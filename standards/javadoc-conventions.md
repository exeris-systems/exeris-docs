...
12. **Javadoc carries no history.** The rule is [`comment-conventions.md`](comment-conventions.md): a comment states the invariant, not its origin, and the history goes to the `CHANGELOG`, the release notes or an ADR. What Java adds is the tags — `@since` (when the element appeared) and `@deprecated` (since when, and what replaces it) are the only temporal ones, and an invariant that deserves a *why* at length gets `@see ADR-NNN` rather than a paragraph. `[L1 (warning): Checkstyle RegexpSingleline, over the token list in exeris-systems/.github → comment-history.json]` `[L2]`
    - **Not every why is an ADR.** The address depends on what the why is about. A why about the *contract* — why a caller must do it this way — is `@apiNote`. A why about *this* implementation — why the window is 8 KiB, why the loop re-reads — is `@implNote`. A why that spans more than one type belongs in the subsystem document. An ADR is for a why that settles a question someone could reasonably have answered differently, and `@see ADR-NNN` links it. What [`comment-conventions.md`](comment-conventions.md) rule 3 forbids is the paragraph that *narrates*; a sentence that *explains* has a home, and this bullet says which. `[L2]`
