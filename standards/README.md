...
| [`comment-conventions.md`](comment-conventions.md) | comments in every language: what a comment carries and what belongs in a commit, an issue or an ADR | `exeris-systems/.github` → `comment-history.json`, `scripts/comment_history_check.py`, `java/checkstyle-javadoc.xml`, `ts/eslint.tsdoc.mjs` |
...
| [`eca-specification.md`](eca-specification.md) | The Exeris Contract Architecture — what an entitlement permits, how it is proved offline, and the taxonomy the manifest draws on | `[L2]` per ADR-085 §M.38; the manifest schema and the runtime record are ADR-088's and ADR-089's |
...
| [`sku-conventions.md`](sku-conventions.md) | Tier 3 Platform SKU repositories (`exeris-sku-*`): `composition.json`, kernel-direct execution, driver-swap transparency, default JVM baseline | `exeris-sdk-composition-spec`, `CompositionStampAssertion`, `exeris-tooling` |
