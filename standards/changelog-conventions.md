...
2. **`Breaking` agrees with the compatibility tool.** Every japicmp/revapi-reported incompatible change on a `stable` surface appears under `Breaking` with its ADR; a `Breaking` entry without a report line, or a report line without an entry, fails the release gate. `[L1: release-impact-check step; ADR-065]`
