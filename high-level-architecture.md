...
| Distributed saga orchestration **in-process** | L4 Flow Engine with off-heap state; durable state in the application's own datastore via `FlowSnapshotStore` | ADR-013, ADR-022 | **No coordinator process and no distributed lock service** (ADR-013 §3 — a single-leader coordinator with consensus is the recorded rejected alternative). Deployment labels: *in-process* + *store-backed*, against *server-backed* and *dedicated* elsewhere in the field. Countable by the buyer from the deployment diagram. |
